...
(function ($) {
	...
	function launch(elem) {
		...
		if (!open) {
			...
			if (settings.overlayClose) {
				$overlay.css({"cursor": "pointer"}).one('click.colorbox', $.colorbox.close);
			}
	...
	$.fn.colorbox = function (options, callback) {
		var $this = this;
		if( options == 'destroy' ) {
			$(this).removeData(colorbox).removeClass('cboxElement');
			return this;
		}
	...
	$.colorbox = {
		init: function () {
			...
			$next.click($.colorbox.next);
			$prev.click($.colorbox.prev);
			$close.click($.colorbox.close);

			// Adding the 'hover' class allowed the browser to load the hover-state
			// background graphics.  The class can now can be removed.
			$content.children().removeClass(hover);

			$('.cboxElement').live('click.colorbox', function (e) {
		...
		resize: function (object) {
			...
			if (isIE6) {
				$('select:not(#colorbox select)').filter(
					function(){
		...
		load: function () {
			var href, img, setResize, resize = $.colorbox.resize;
 ...
 */
			...
			if (settings.inline) {
				...
				img.onload = function(){
					...
					if($related.length > 1){
						$(img).css({cursor:'pointer'}).click($.colorbox.next);
					}
		...
		slideshow: function () {
			...
			function start(){
				...
				.bind(cbox_complete, 
					function(){
						timeOut = setTimeout($.colorbox.next, settings.slideshowSpeed);
					}
				)
				.bind(cbox_load, 
					function(){
